...
func (service *Service) CreateSubsriptionInfo(ctx *gin.Context, moniest_id string, req model.CreateMoniestRequest) (db.SubscriptionInfo, error) {
	...
	if err := validation.Fee(req.Fee, service.config); err != nil {
		return db.SubscriptionInfo{}, clientError.CreateError(http.StatusNotAcceptable, clientError.Moniest_CreateSubscriptionInfo_InvalidFee)
	}
	...
	if req.Message != "" {
		if err := validation.SubscriptionMessage(req.Message, service.config); err != nil {
			return db.SubscriptionInfo{}, clientError.CreateError(http.StatusNotAcceptable, clientError.Moniest_CreateSubscriptionInfo_InvalidSubscriptionMessage)
		}
